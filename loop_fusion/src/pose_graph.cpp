...
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    if (DEBUG_IMAGE)
    {
        ...
        putText(compressed_image, "feature_num:" + to_string(feature_num), cv::Point2f(10, 10), font, 0.4, cv::Scalar(255));
    ...
    if (DEBUG_IMAGE)
    {
        ...
        if (ret.size() > 0)
            putText(loop_result, "neighbour score:" + to_string(ret[0].Score), cv::Point2f(10, 50), font, 0.5, cv::Scalar(255));
    }
    // visual loop result 
    if (DEBUG_IMAGE)
    {
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            ...
            putText(tmp_image, "index:  " + to_string(tmp_index) + "loop score:" + to_string(ret[i].Score), cv::Point2f(10, 50), font, 0.5, cv::Scalar(255));
        ...
        for (unsigned int i = 1; i < ret.size(); i++)
        {
            ...
            {          
                ...
                if (DEBUG_IMAGE && 0)
                {
                    auto it = image_pool.find(tmp_index);
                    cv::Mat tmp_image = (it->second).clone();
                    putText(tmp_image, "loop score:" + to_string(ret[i].Score), cv::Point2f(10, 50), font, 0.4, cv::Scalar(255));
                    cv::hconcat(loop_result, tmp_image, loop_result);
                }
            }
...
//        printf("path p: %f, %f, %f\n",  P.x(),  P.z(),  P.y() );
        ...
        //     loop_path_file.close();
        // }
        if (SAVE_LOOP_PATH) // TUM RGB-D format
        {
            ofstream loop_path_file(VINS_RESULT_PATH, ios::app);
            loop_path_file.setf(ios::fixed, ios::floatfield);
            loop_path_file.precision(5);
